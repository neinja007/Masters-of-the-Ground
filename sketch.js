...
function draw() {
	...
	}
	{
		...
		if (millis() > GAME.frame_update + DATA.f_upd_int) {
			...
			// punch
			if (GAME.p1.status === 'punch') {
				GAME.p1.cooldowns.punch = DATA.cld_pun;
			}
			if (GAME.p2.status === 'punch') {
	...
	}
	{
		...
		if (
			GAME.p1.down_held &&
			!GAME.key_status.includes(83) &&
			GAME.p1.cooldowns.shield == 0 &&
			GAME.p1.status == 'default'
		) {
		...
		if (
			GAME.p2.down_held &&
			!GAME.key_status.includes(40) &&
			GAME.p2.cooldowns.shield == 0 &&
			GAME.p2.status == 'default'
		) {
		...
		if (
			GAME.p1.up_held &&
			GAME.key_status.includes(68) &&
			GAME.p1.cooldowns.punch <= 0 &&
			GAME.p1.status != 'charge_4'
		) {
			GAME.p1.status = 'punch';
			GAME.p1.status_index = 1;
		...
		if (
			GAME.p2.up_held &&
			GAME.key_status.includes(39) &&
			GAME.p2.cooldowns.punch <= 0 &&
			GAME.p2.status != 'charge_4'
		) {
			...
			for (let i of ['p1', 'p2']) {
				...
				if (!GAME[i].hit) {
					if (GAME[i].status === 'punch') {
						if (
							(GAME[i].direction == 'left' && GAME[j].x + 10 < GAME[i].x && GAME[j].x + 90 > GAME[i].x) ||
							...
							if (GAME[j].status.includes('charge')) {
								if (GAME[i].status.split('_')[1] > GAME[j].status.split('_')[1]) {
									...
									if (GAME[i].status_index < GAME[j].status_index) {
										if (
											GAME[i].status !== 'charge_4' ||
											(GAME[i].status === 'charge_4' && GAME[i].status_index === 1)
										) {
											GAME[i].hit = true;
											GAME[j].status = 'default';
											GAME[j].lives -= DATA['dmg_ch' + GAME[i].status.split('_')[1]];
										...
										if (
											GAME[j].status !== 'charge_4' ||
											(GAME[j].status === 'charge_4' && GAME[j].status_index === 1)
										) {
											GAME[j].hit = true;
											GAME[i].status = 'default';
											GAME[i].lives -= DATA['dmg_ch' + GAME[j].status.split('_')[1]];
	...
	}
	{
		...
		if (GAME.p1.x < DATA.bd_l) {
			if (GAME.p1.status.includes('charge')) {
				GAME.p1.cooldowns.charge = DATA.cld_chr;
			}
			GAME.p1.x = DATA.bd_l;
			GAME.p1.status = 'default';
		...
		if (GAME.p2.x < DATA.bd_l) {
			if (GAME.p2.status.includes('charge')) {
				GAME.p2.cooldowns.charge = DATA.cld_chr;
			}
			GAME.p2.x = DATA.bd_l;
			GAME.p2.status = 'default';
	...
	}
	{
		...
		if (GAME.p1.wins == 10 || GAME.p2.wins == 10) {
